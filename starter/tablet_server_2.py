...
def metadata_for_max_size(size, tablet_server_name):
        ...
        if content_length != None:
            ...
            if len(request_path.split("/")) >= 3:
                ...
                # Reset memtable size
                    ...
                    # t1 = Thread(target=self.recover_from_other, args=dead_tablet)
                    # t1.start()
                    print("read from other dead tablet")
                    self._set_response(200)
                check_tables()

    def recover_from_other(self, dead_tablet):
        print("recover from other")
